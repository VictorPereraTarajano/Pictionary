...
public class CheckWordCommand implements Command {
    ...
    public void execute() {
        if (ManagerGame.GAME_STATE == ManagerGame.IN_GAME && WordMatcher.match(word, ManagerLobby.myLobby.getGame().currentTurn().getWord())) {
            ...
            if (ManagerLobby.myLobby.getGame().currentTurn().isFinished()) new SendCommand(new StopTurnCommand("Turno finalizado"), ManagerConnection.TCPBroadcast(new Player[] {ManagerLobby.myLobby.getHost()})).execute();
